...
def _maybe_wrap_and_compile(
    model_: GraphModule,
    example_inputs_: Sequence[InputType],
    compile_gm: Callable[[GraphModule, Sequence[InputType]], CompileFxOutput],
) -> Optional[CompileFxOutput]:
    """
    Called by _compile_fx_main before doing actual work.
    Under various conditions, different kinds of wrapping might be needed.
    In such cases, do the wrapping and then recursively call _compile_fx_main.
    Otherwise, return None so that actual work can happen in _compile_fx_main.
    """
...
def _compile_fx_main(
    ...
    """
    Main part of compile_fx, called after patching configs.
    """
    maybe_compiled_fn = _maybe_wrap_and_compile(
        model_,
        example_inputs_,
        functools.partial(
            _compile_fx_main,
    ...
    if maybe_compiled_fn is not None:
        return maybe_compiled_fn
